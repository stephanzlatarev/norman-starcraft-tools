...
export default class Map {
  ...
  constructor(gameInfo, observation) {
    this.units = observation.observation.rawData.units.filter(unit => (unit.owner !== 1) && (unit.owner !== 2)).map(unit => ({
      tag: unit.tag,
      unitType: unit.unitType,
      radius: unit.radius,
      x: unit.pos.x,
      y: unit.pos.y,
      isMinerals: (unit.radius === 1.125),
      isVespene: (unit.radius === 1.5),
    }));
    ...
    const minerals = this.units.filter(unit => !!unit.isMinerals);
    const vespenes = this.units.filter(unit => !!unit.isVespene);
